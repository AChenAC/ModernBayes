...
## Plots
```{r}
data_frame(idx = 1:length(db), db = db) %>%
...
data_frame(idx = 1:length(norm_diff_sigma), norm_diff_sigma = norm_diff_sigma, norm_diff_mu = norm_diff_mu) %>%
  gather(key, value, norm_diff_sigma, norm_diff_mu) %>%
  ggplot(aes(x=idx, y=value, colour=key)) + geom_line(size = 1.2)  + scale_y_continuous(trans='log10')
```
